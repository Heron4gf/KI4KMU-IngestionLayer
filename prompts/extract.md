...
4. **concepts**: An array of concepts that categorize this section. Concepts must follow the quality rules below.
...
**Expected output:**
```json
{"sections": [{"section_id": "engine_compression_ratio", "label": "Engine Compression Ratio", "texts": [{"content": "compression ratio Chassis 3413G between 1 and 3 bars"}], "concepts": [{"label": "chassis"}, {"label": "compression"}]}]}
```
...
**Expected output:**
```json
{"sections": [{"section_id": "cpu_specifications", "label": "2.4 CPU Specifications", "texts": [{"content": "Ryzen 9 5950X operates at base clock 3.4 GHz boost clock 4.9 GHz"}], "concepts": [{"label": "processor"}, {"label": "clock speed"}]}]}
```
...
**Expected output:**
```json
{"sections": [{"section_id": "neck_profile_dimensions", "label": "Figure 5: Neck Profile Dimensions", "texts": [{"content": "Figure 5: Neck Profile Dimensions"}], "concepts": [{"label": "figure"}]}]}
```
...
- Use the lemma (base form): `"transparent"` not `"transparency"`; `"apply"` not `"application"` — unless the noun is the canonical search term in context (e.g. `"application"` in a software context is fine).
...
- Proper nouns (product names, company names, standards) are kept as-is since they are exact-match retrieval targets: `"GDPR"`, `"AMD Ryzen"`, `"ISO 9001"`.
